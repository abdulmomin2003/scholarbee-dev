...
import {
  ...
  Avatar, Divider, FormControlLabel
} from '@mui/material';
...
export default function MLDashboard() {
  ...
  const handleTestModel = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3010/api/admin/ml/test', {
        headers: { 'Authorization': \`Bearer \${Cookies.get('access_token')}\` }
      });
      const data = await res.json();
            ...
            <Box sx={{ mt: 4 }}>
              ...
              <Grid container spacing={3}>
                {testModelResult.results.map((prog: any, idx: number) => (
                  <Grid item xs={12} md={6} lg={4} key={prog.program_id}>
...
`;
fs.writeFileSync('d:/STUDY/Fiver/scholarbee-dev/scholarbee-dev/student-portal/src/app/admin/ml-dashboard/page.tsx', code);
