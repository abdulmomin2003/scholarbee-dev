...
const UniversitySearchDropdown = ({
  ...
  const memoizedDefaultValue = useMemo(() => {
    ...
    const id = defaultValue.id || (defaultValue as { _id?: string })?._id || '';
  ...
  const renderInput = useCallback(
    (params: AutocompleteRenderInputParams) => (
      <TextField
        ...
        placeholder={showPlaceholder ? 'Select University' : ''}
        name="university11"
        variant={variant}
        sx={{
          backgroundColor:
            onComparePage || onProgramPage ? COLORS.white : '#F7F8F9',
          borderRadius: variant === 'filled' ? '8px' : '12px',
          ...(variant === 'filled' && !onComparePage && !onProgramPage
            ? classes.noBorder
            : {})
        }}
        InputProps={{
    ...
    >
      ...
      <Autocomplete
        ...
        isOptionEqualToValue={(option, value) => {
          ...
          // Handle both { id, name } and { _id, name } formats
          const optionId = option._id || (option as { id?: string })?.id || '';
          const valueId = value._id || (value as { id?: string })?.id || '';
          return optionId && valueId ? optionId === valueId : false;
        }}
