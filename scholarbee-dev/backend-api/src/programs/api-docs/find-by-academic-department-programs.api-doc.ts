...
export function FindProgramsByAcademicDepartmentApiDoc() {
  return applyDecorators(
    ApiOperation({ summary: 'Find programs by academic department' }),
    ApiParam({ name: 'academicDepartmentId', required: true, type: String }),
    ...
    ApiResponse({ status: 200, description: 'Paginated programs for department', schema: { example: { data: [], meta: { total: 0, page: 1, limit: 10, pages: 0 } } } }),
  );
}
